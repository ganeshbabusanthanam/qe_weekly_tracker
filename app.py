    ...
    if generate_report:
        ...
        try:
            ...
            if data:
                ...
                html = f"<h2 style='text-align:center;'>Weekly Report</h2>"
                ...
                for idx, (pname, details) in enumerate(project_data.items()):
                    ...
                    <div style="{'page-break-before: always;' if idx > 0 else ''} font-family: Arial, sans-serif; padding: 20px;">
                        ...
                        <p style="margin: 0;"><strong>Client/BU:</strong> {details['client_business_unit']}</p>
                        <p style="margin: 0;"><strong>Project Manager:</strong> {details['project_manager']}</p>
                        <p style="margin: 0;"><strong>Duration:</strong> {details['start_date']} to {details['end_date']}</p>
                        <p style="margin-bottom: 10px;"><strong>Phase:</strong> {details['current_phase']} | <strong>Status:</strong> <span style="color:{'green' if details['status_indicator']=='On Track' else 'red'}">{details['status_indicator']}</span></p>

                        <h4 style="color:#004080;">Accomplishments</h4>
                        <ul>{"".join([f"<li>{line.strip()}</li>" for line in set(details['accomplishments'].splitlines()) if line.strip()])}</ul>

                        <h4 style="color:#004080;">Decisions Needed</h4>
                        <ul>{"".join([f"<li>{line.strip()}</li>" for line in set(details['decisions_needed'].splitlines()) if line.strip()])}</ul>
                        ...
                        <ul>{"".join([f"<li><b>{ri['type']}</b>: {ri['description']} (Owner: {ri['owner']}, ETA: {ri['mitigation_eta']})</li>" for ri in set(tuple(d.items()) for d in details['risks_issues'])]) or "<li>No risks or issues</li>"}</ul>

                        <h4 style="color:#004080;">Action Items</h4>
                        <ul>{"".join([f"<li>{a['description']} - {a['status']} (Client Input: {'Yes' if a['client_input_required'] else 'No'})</li>" for a in set(tuple(d.items()) for d in details['action_items'])]) or "<li>No action items</li>"}</ul>
                    </div>
                    """
